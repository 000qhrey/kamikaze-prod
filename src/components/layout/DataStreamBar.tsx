...
export function DataStreamBar() {
  ...
  // Update dynamic rave values — throttled site-wide
  useEffect(() => {
    if (lite) return

    const bassInterval = setInterval(() => {
      setDynamicValues((prev) => ({ ...prev, bass: generateBassBar(), db: generateDB() }))
    }, PERF.dataStreamBassMs)
  ...
  // Occasional glitch accents — reduced frequency on all devices
  useEffect(() => {
    if (lite) return
    ...
    const glitchInterval = setInterval(() => {
      if (Math.random() > 0.6) {
        ...
        setGlitchIndices(new Set([idx]))
        setTimeout(() => setGlitchIndices(new Set()), 80)
      }
    }, PERF.dataStreamGlitchMs)
            ...
            <div key={dupeIndex} className="flex items-center gap-6 px-4">
              ...
              {STATIC_CODES.map((code, index) => (
                <DataItem
                  ...
                  isGlitching={!lite && glitchIndices.has(index)}
                  isCorrupted={!lite && corruptedIndex === index}
                />
              ))}
              ...
              <span
                className="font-mono text-[10px] text-yellow-500/50"
